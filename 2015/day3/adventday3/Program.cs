...
namespace adventday3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Part 1
            Santa soloSanta = new Santa("Fatboy", startingPoint);
            input.ToList().ForEach(x => soloSanta.Fly(x));

            Console.WriteLine("PART 1");
            Console.WriteLine("-------");
            Console.WriteLine("Total Moves: {0}, Distinct Houses: {1}", soloSanta.FlightHistory.Count, soloSanta.FlightHistory.Distinct().Count());
            Console.WriteLine("");

            //Part 2
            Santa santa = new Santa("Fatboy", startingPoint);
            Santa roboSanta = new Santa("Metalboy", startingPoint);
            ...
            for(var i = 0; i < input.ToList().Count; i++)
            {
                if (i % 2 == 0) {
                    ...
                    roboSanta.Fly(input[i]);
                }
            }

            Console.WriteLine("PART 2");
            Console.WriteLine("-------");
            Console.WriteLine("Santa Moves: {0}, Santa Distinct Houses: {1}", santa.FlightHistory.Count, santa.FlightHistory.Distinct().Count());
            Console.WriteLine("Robo-Santa Moves: {0}, Robo-Santa Distinct Houses: {1}", roboSanta.FlightHistory.Count, roboSanta.FlightHistory.Distinct().Count());
            Console.WriteLine("Joint Distinct Houses: {0}", santa.FlightHistory.Union(roboSanta.FlightHistory).Count());
    ...
    class Santa
    {
        ...
        private Coordinates StartingPoint { get; set; }
        public List<Coordinates> FlightHistory { get; private set; }
        public Coordinates CurrentPosition { get; private set; }


        public Santa(string name, Coordinates startingPoint)
        {
            Name = name;
            StartingPoint = startingPoint;
            FlightHistory = new List<Coordinates> {
                startingPoint
            };
        }
